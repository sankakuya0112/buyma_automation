...
## 🆕 2026-09-23 追記: 原価計算のズレ 2 件を修正 (利益の過大評価)

どちらも「原価を少なく見積もる = 赤字出品になり得る」方向のズレだった。

### 修正内容
- **革靴の関税**: 30% または 1 足 ¥4,300 の高い方 (`app/core/pricing.py:resolve_duty`)。
  以前は product_type "FOOTWEAR" が税率表に当たらず既定 10% に落ちていた。
  靴の判定は product_type の**単語単位** (Bootcut Jeans / Oxford Shirt を誤判定しない)。
  タイトルに布系の語 (canvas / knit / mesh …) があり革系の語が無い靴だけ従来税率、
  **素材不明の靴は革扱い** (安全側)
- **通関の立替手数料**: DDU 仕入れで max(¥2,200, 2% × (関税+消費税)) を総原価に加算。
  `BaseSource.customs_handling_min_jpy / customs_handling_rate` が既定値、
  `data/sources.json` で仕入先ごとに上書き可。DDP (italist 等) は 0
- 出力 CSV に `customs_handling_jpy` 列を追加 (`consumption_tax_jpy` の直後)
- 調査レポートの「¥3,300」は発送人払いの料金との取り違え → ¥2,200 に訂正
- テスト 466 → **489 ケース全 PASS**

### 影響 (baseblu、修正前の売価のまま出品した場合の利益)
| 商品 | 原価の増加 | 利益 |
|---|---|---|
| 革ローファー €600 | +¥24,698 | ¥31,971 → ¥7,273 |
| 革スニーカー €350 | +¥16,176 | ¥19,960 → ¥3,784 |
| バッグ €1,200 | +¥2,200 | ¥56,795 → ¥54,595 |
| ニット €400 | +¥2,200 | ¥22,381 → ¥20,181 |
...
### 未対応 (次の候補)
- ~~原価モデルの是正 (立替手数料・革靴の関税)~~ → 2026-09-23 対応済み (上の追記参照)
